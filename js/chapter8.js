...
function validatePassword() {
   ...
   try {
     ...
     if (/.{8}/.test(pw1Input.value) === false) {
       ...
       throw 'Password must contain al leaset one number';
...
// add newsletter to profile
function registernewsletter(event) {
   if (event === undefined) { // get caller element in IE8
      event = window.event;
   }
   var callerElement = event.target || event.srcElement;
   var newsletterName = callerElement.value;
   if (callerElement.checked) { // if box has just been checked
     // add checbox value to newsletter array
     newsletter.push(newsletterName);


      // add checkbox value to list in profile section
      var newnewsletter = document.createElement("li");
      newnewsletter.innerHTML = newsletterName;
      document.getElementById("profilenewsletters").appendChild(newnewsletter);
      // make profile section and newsletter section visible
      document.getElementById("profile").style.display = "block";
      document.getElementById("newslettersSection").style.display = "block";
   } else { // if box has just been unchecked
      var listItems = document.querySelectorAll("#profilenewsletters li");
      for (var i = 0; i < listItems.length; i++) {
         if (listItems[i].innerHTML === newsletterName) {
           // remove element at index i from array
           newsletter.splice(i, 1);


            // remove newsletter from profile list
...
function convertToString() {
  // convert newsletter array to string
  arrayString = newsletter.toString();

  // convert profile object to sting
...
function createEventListeners() {
   ...
   var newsletters = document.getElementsByName("newsletters");
   if (newsletters[0].addEventListener) {
      for (var i = 0; i < newsletters.length; i++) {
         newsletters[i].addEventListener("change", registernewsletter, false);
      }
   } else if (newsletters[0].attachEvent) {
      for (var i = 0; i < newsletters.length; i++) {
         newsletters[i].attachEvent("onchange", registernewsletter);
      }
   }
   ...
   if (button.addEventListener) {
     ...
     buton.attachEvent('onclick', convertToString);
   }
}
